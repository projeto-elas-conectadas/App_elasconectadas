...
Funcionalidade: Autenticação
  ...
  Cenário: Login rejeita corpo sem campos obrigatórios
    Quando eu envio POST para "/login" com o corpo:
      """
      {}
      """
  ...
  Cenário: Send rejeita corpo sem e-mail
    Quando eu envio POST para "/send" com o corpo:
      """
      {}
      """
    Então a resposta deve ter status 400
  ...
  Cenário: Request OTP rejeita e-mail inválido
    Quando eu envio POST para "/request-otp" com o corpo:
      """
      {"email": "invalido"}
      """
    Então a resposta deve ter status 400
  ...
  Cenário: Verify OTP rejeita corpo incompleto
    Quando eu envio POST para "/verify-otp" com o corpo:
      """
